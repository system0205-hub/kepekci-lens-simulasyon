...
function checkSpecialManufacture(diopter) {
...
function getReferenceThickness(index, diopter) {
...
function getThicknessAt6D(index) {
  const ref = THICKNESS_REFERENCES[index];
  if (!ref || !ref['6.00']) return null;
  return ref['6.00'];
}

function interpolateInRange(references, targetDiopter, minDiopter, maxDiopter) {
...
function interpolateHighPower(thicknessAt6D, diopter) {
  const exponent = 1.15;
...
function applyIndexFactor(thickness, targetIndex) {
...
function applyFrameEffect(thickness, frameSize) {
  if (!thickness) return null;
...
function applyCylEffect(baseThickness, cyl, axis) {
  if (!baseThickness) return null;
...
function getThinnessScore(thickness, lensIndex) {
...
function calculateTotalDiopter(prescription) {
  const maxSph = Math.max(
    Math.abs(prescription.right.sph || 0),
    Math.abs(prescription.left.sph || 0)
  );
...
export { getReferenceThickness, applyIndexFactor, applyFrameEffect, applyCylEffect, checkSpecialManufacture, getThinnessScore, calculateTotalDiopter };
